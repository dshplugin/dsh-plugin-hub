...
import { activeTask, cancelTask, dumpLoaderEntries, getTask, githubTarget, hasQueuedTarget, listPendingRestarts, readProfileArg, startPluginMutation, validPackageName } from '../services/install.js';
import { recordInstalledVersion, recordResolvedNpmPackage, readInstalledVersions, removeInstalledVersion } from '../services/installed-versions.js';
import { resolveNpmPackage } from '../services/npm-resolve.js';
import { preflightTarget } from '../services/preflight.js';
...
function installedByRepo(installed, target) {
    ...
    for (const [name, spec] of Object.entries(installed)) {
        if (spec.toLowerCase().includes(needle))
            return name;
    }
    return null;
}
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const mode = body !== null && typeof body === 'object' && body.mode === 'update'
                        ? 'update'
                        : 'install';
                    ...
                    let target = repoTarget ?? (repoTarget === null && validPackageName(rawRepo) ? rawRepo : null);
                    if (target === null) {
