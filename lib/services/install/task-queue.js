...
import { npmTooLowMarker } from './npm-check.js';
import { cleanLine, estimateProgress } from '../profile/progress.js';
import { removeLoadedEntry } from '../loader.js';
import { addPendingRestart, clearPendingRestart } from '../profile/pending-restart.js';
import { addAllowBuildsKey, githubRepoOf, parseAllowBuildsKey, profileDirectory } from '../profile/profile.js';
...
export function activeTask() {
    ...
    const show = (task) => task.displayTarget ?? task.target;
...
export function startPluginMutation(options) {
    const task = { id: nextTaskId, target: options.target, displayTarget: options.displayTarget, globalNpm: options.globalNpm, action: options.action, status: 'pending', timedOut: false, exitCode: null, progress: 0, lines: [], attempts: options.attempts ?? [], needsRestart: false, installChannel: options.installChannel };
...
function pumpQueue() {
    ...
    void runPluginMutation({ ...options, task }).finally(() => {
        ...
        const verb = (v) => task.action === 'remove' ? `卸载${v}` : task.action === 'update' ? `更新${v}` : `安装${v}`;
        if (task.status === 'done') {
            appendLog(options.profile, {
                at: Date.now(),
                level: 'success',
...
function spawnMutation(options) {
    ...
    const spawnOnce = (attemptArgs) => new Promise((resolveOnce) => {
        let stdout = '';
        let stderr = '';
        let timedOut = false;
        let settled = false;
    ...
    return (async () => {
        ...
        if (isGlobalNpm && result.exitCode !== 0 && !result.timedOut && task?.status !== 'cancelled') {
            ...
            if (task && !task.attempts.includes(retryExecuted))
                task.attempts.push(retryExecuted);
            if (task)
                pushLine(task, '[npm] peer-set resolution crashed npm (known npm bug) — retrying with --legacy-peer-deps');
...
function annotateNpmTooLow(result, options) {
    if (options.action === 'remove' || result.exitCode === 0 || result.timedOut)
        return result;
    const line = npmTooLowMarker(`${result.stderr}\n${result.stdout}`, options.env);
    if (line === null)
        return result;
    if (options.task && options.task.status !== 'cancelled')
        pushLine(options.task, line);
...
export async function runPluginMutation(options) {
    ...
    if (options.action !== 'remove' && result.exitCode !== 0 && !result.timedOut && (options.globalNpm ?? []).length === 0) {
        ...
        if (result.exitCode !== 0 && !result.timedOut && `${result.stderr}\n${result.stdout}`.includes('ERR_PNPM_IGNORED_BUILDS')) {
            ...
            if (keys.length === 0 && githubRepoOf(options.target) === null)
                keys.push(options.target);
            if (keys.length > 0) {
                if (options.task) {
                    options.task.status = 'running';
                    options.task.exitCode = null;
