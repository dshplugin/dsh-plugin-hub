...
const CATEGORIES = ['install', 'uninstall', 'update', 'diagnostics', 'settings', 'system'];
 ...
 */
export function customLogFile(override) {
 ...
 */
export function defaultLogFilePath(profile) {
 ...
 */
export function logFilePath(profile) {
...
export function clearLog(profile) {
    try {
        const file = logFilePath(profile);
        mkdirSync(dirname(file), { recursive: true });
        writeFileSync(file, '');
...
function normalize(raw) {
    ...
    if (typeof e.at !== 'number')
        return null;
    let level = 'info';
    if (e.level === 'ok' || e.level === 'success')
        level = 'success';
    else if (e.level === 'fail' || e.level === 'error')
        level = 'error';
    else if (e.level === 'warn')
        level = 'warn';
    else if (e.level === 'debug')
        level = 'debug';
    const category = CATEGORIES.includes(e.category) ? e.category : 'system';
    return {
        at: e.at,
        level,
        category,
...
export function readLog(profile, opts = {}) {
