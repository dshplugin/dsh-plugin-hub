 ...
 *   loader 无残留，无需再提醒重启，直接移除该条；否则（此前已被宿主加载过）登记「卸载后待重启清理」。
 * - 安装：已有记录则刷新为安装语义（如卸载后未重启又重装），否则新登记。
 */
export function addPendingRestart(target, kind) {
    const existing = pendingRestarts.find((p) => p.target === target);
    if (kind === 'uninstall') {
        if (existing)
            pendingRestarts.splice(pendingRestarts.indexOf(existing), 1);
        else
