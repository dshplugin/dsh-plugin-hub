...
export function tlsRevocationBlocked(stderr) {
...
function runCurl(args, env, timeoutMs) {
...
async function curlWithTlsFallback(args, env, timeoutMs) {
    const tlsArgs = curlTlsArgs();
    const first = await runCurl([...tlsArgs, ...args], env, timeoutMs);
    if (tlsArgs.length === 0 || !curlRevokeFlagUnsupported(first.code, first.stderr))
        return first;
 ...
 */
async function curlProbe(url, proxy, timeoutMs) {
 ...
 */
export function probeUrl(url, proxy, timeoutMs) {
    let target;
    try {
        target = new URL(url);
    }
    catch {
        return Promise.resolve(probeFail());
    }
 ...
 */
export function fetchViaCurl(url, proxy, timeoutMs) {
