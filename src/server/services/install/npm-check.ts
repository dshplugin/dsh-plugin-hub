...
function belowMin(v: [number, number, number]): boolean {
  for (let i = 0; i < NPM_MIN_VERSION.length; i++) {
    if (v[i] < NPM_MIN_VERSION[i]) return true
    if (v[i] > NPM_MIN_VERSION[i]) return false
  }
  return false
}
 ...
 */
export function npmTooLowMarker(output: string, env?: NodeJS.ProcessEnv): string | null {
  if (!NPM_CRASH_EDGES_OUT_RE.test(output)) return null
  const v = npmVersionOf(env)
  if (v === null || !belowMin(v)) return null
