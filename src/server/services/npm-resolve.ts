 ...
 * 从而把「git 直装必败」的插件切换到 npm 通道安装。命中结果带内存缓存，
 ...
 * 原有错误路径，反查只是额外的一次尝试。
 */
export function resolveNpmPackage(repo: string): Promise<string | null> {
  const key = repo.toLowerCase()
  if (cache.has(key)) return Promise.resolve(cache.get(key) ?? null)
  const name = searchRepo(repo)
  name.then((found) => cache.set(key, found)).catch(() => cache.set(key, null))
  return name
}

/** 用 npm search 接口按 repository 地址反查，并做铁证校验（包元数据必须指向该仓库）。 */
function searchRepo(repo: string): Promise<string | null> {
  const [owner, name] = repo.split('/')
  if (owner === undefined || name === undefined || name === '') return Promise.resolve(null)
  const url = `https://registry.npmjs.org/-/v1/search?text=${encodeURIComponent(`repository:${owner}/${name}`)}&size=10`
